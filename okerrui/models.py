...
class CheckMethod(models.Model):
    ...
    def action_logic(self, i):
        ...
        dump = i.getarg('dump', '')
        safenodes = ['Constant', 'Expression', 'Subscript', 'Name', 'Load', 'Compare']
    ...
    def cron(force=False):
        ...
        if now > lastrun + 7200 or force:
            for profile in Profile.objects.filter(ci=myci()):
                ...
                if deleted[0]:
                    log.info("clean updatelog for profile {} deleted {} (retention: {}, {})".format(
                        profile, deleted[0], retention, shortdate(before)))
